...
router.get("/get-all-tasks", authanticateToken , async(req, resp)=>{
  try {
    const {id} = req.headers ; 
   const userData = await User.findById(id).populate({path:"tasks",options:{sort:{createdAt:-1}}}); //populate := for display all task with there data and options sort the task in the way the latest task show in first
